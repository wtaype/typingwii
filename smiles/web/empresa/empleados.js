// ════════════════════════════════════════════════════════════════════
// empleados.js — TypingWii · Empresa · Colaboradores
// Jesús es mi Señor 🙏
// ════════════════════════════════════════════════════════════════════
import './empleados.css';
import $ from 'jquery';
import { getls, savels, Notificacion, avatar } from '../../widev.js';
import { db } from '../firebase.js';
import { collection, query, where, getDocs } from 'firebase/firestore';

const wi = () => getls('wiSmile');

// ── RENDER ────────────────────────────────────────────────────────────────────
export const render = () => {
  const u = wi();
  if (!u) return `<div class="ep_page"><div class="ep_empty"><i class="fas fa-lock"></i><p>Sin sesión corporativa.</p></div></div>`;

  return `
  <div class="ep_page">

    <!-- HERO CORPORATIVO PRO -->
    <div class="ep_hero">
      <div class="ep_hero_left">
        <div class="ep_hero_icon"><i class="fas fa-building"></i></div>
        <div class="ep_hero_txt">
          <div class="ep_badge"><i class="fas fa-users"></i> Panel Empresarial</div>
          <h1 class="ep_hero_title">Gestión de Equipo</h1>
          <p class="ep_hero_sub">Administra y evalúa el entrenamiento de tus colaboradores.</p>
        </div>
      </div>
      <div class="ep_hero_actions">
        <button class="ep_btn_primary" id="ep_btn_invitar">
          <i class="fas fa-user-plus"></i> Añadir Colaborador
        </button>
      </div>
    </div>

    <!-- CONTROLES -->
    <div class="ep_controls">
      <div class="ep_tabs">
        <button class="ep_tab active" data-f="todos">Todos</button>
        <button class="ep_tab" data-f="activos">Activos</button>
        <button class="ep_tab" data-f="inactivos">Inactivos</button>
      </div>
      <div class="ep_search">
        <i class="fas fa-search"></i>
        <input type="text" id="ep_search_inp" placeholder="Buscar por nombre o correo...">
      </div>
    </div>

    <!-- GRID DE EMPLEADOS -->
    <div class="ep_grid" id="ep_grid">
      <div class="ep_empty" style="grid-column: 1 / -1">
        <i class="fas fa-spinner fa-spin"></i><p>Cargando nómina...</p>
      </div>
    </div>

    <!-- CONTENEDOR MODALES -->
    <div id="ep_modales"></div>

  </div>`;
};

// ── ESTADO ────────────────────────────────────────────────────────────────────
let _empleados = [];
let _equipos   = [];

// ── INIT ──────────────────────────────────────────────────────────────────────
export const init = async () => {
  const u = wi();
  if (!u) return;

  $(document).off('.ep');
  await _cargarDatos(u);

  // Filtros
  $(document).on('click.ep', '.ep_tab', function () {
    $('.ep_tab').removeClass('active');
    $(this).addClass('active');
    _renderGrid();
  });

  // Buscador
  let debounceT;
  $(document).on('input.ep', '#ep_search_inp', function () {
    clearTimeout(debounceT);
    debounceT = setTimeout(() => _renderGrid(), 300);
  });

  // Modales
  $(document).on('click.ep', '.ep_modal_close, .ep_btn_cancel', _cerrarModales);
  $(document).on('click.ep', '.ep_modal_bg', e => { if ($(e.target).hasClass('ep_modal_bg')) _cerrarModales(); });

  // Acciones
  $(document).on('click.ep', '#ep_btn_invitar', _modalInvitar);
  $(document).on('click.ep', '.ep_btn_ver', function () {
    const usr = $(this).data('u');
    const emp = _empleados.find(e => e.usuario === usr);
    if (emp) _modalDetalle(emp);
  });
};

export const cleanup = () => {
  $(document).off('.ep');
};

// ── DATOS ─────────────────────────────────────────────────────────────────────
async function _cargarDatos(u) {
  try {
    let snap = await getDocs(query(collection(db, 'lecciones'), where('empresa_id', '==', u.usuario)));
    if (snap.empty) snap = await getDocs(query(collection(db, 'lecciones'), where('gestor_id', '==', u.usuario)));
    
    _empleados = snap.docs.map(d => ({ usuario: d.id, ...d.data() }));

    let snapEq = await getDocs(query(collection(db, 'clases'), where('empresa_id', '==', u.usuario)));
    if (snapEq.empty) snapEq = await getDocs(query(collection(db, 'clases'), where('gestor_id', '==', u.usuario)));
    _equipos = snapEq.docs.map(d => ({ id: d.id, ...d.data() }));

    _renderGrid();
  } catch (err) {
    console.error('[empleados] Error:', err);
    $('#ep_grid').html('<div class="ep_empty" style="grid-column: 1 / -1"><i class="fas fa-exclamation-triangle"></i><p>Error de conexión al servidor.</p></div>');
  }
}

// ── RENDER GRID (CARDS) ───────────────────────────────────────────────────────
function _renderGrid() {
  const tab = $('.ep_tab.active').data('f');
  const q   = $('#ep_search_inp').val().trim().toLowerCase();

  let lista = _empleados;

  if (tab === 'activos')   lista = lista.filter(e => (e.completadas?.length || 0) > 0);
  if (tab === 'inactivos') lista = lista.filter(e => !(e.completadas?.length || 0));

  if (q) {
    lista = lista.filter(e => {
      const nom = (e.nombre || '').toLowerCase();
      const eml = (e.email || '').toLowerCase();
      return nom.includes(q) || eml.includes(q);
    });
  }

  if (!lista.length) {
    $('#ep_grid').html(`
      <div class="ep_empty" style="grid-column: 1 / -1">
        <i class="fas fa-folder-open"></i><p>No se encontraron colaboradores.</p>
      </div>`);
    return;
  }

  const html = lista.map(e => {
    const nom   = e.nombre || e.usuario || '—';
    const ini   = avatar(nom);
    const eq    = e.equipo_id || e.clase_id || e.claseId || 'General';
    const lecs  = e.completadas?.length || 0;
    const wpm   = e.wpmMax || 0;
    const prec  = e.precision || 0;
    const pct   = Math.round((lecs / 45) * 100);
    const act   = lecs > 0;

    return `
      <div class="ep_card">
        <div class="ep_card_badge ${act ? 'act' : 'inact'}">${act ? 'Activo' : 'Inactivo'}</div>
        <div class="ep_avatar">${ini}</div>
        <div class="ep_card_nombre">${nom}</div>
        <div class="ep_card_email">${e.email || e.usuario}</div>
        <div class="ep_card_equipo"><i class="fas fa-layer-group"></i> ${eq}</div>
        
        <div class="ep_prog_wrap">
          <div class="ep_prog_txt"><span>Avance</span> <span>${pct}%</span></div>
          <div class="ep_prog_bar"><div class="ep_prog_fill" style="width:${pct}%"></div></div>
        </div>

        <div class="ep_card_stats">
          <div class="ep_stat">
            <div class="ep_stat_val" style="color:#f59e0b"><i class="fas fa-bolt"></i> ${wpm}</div>
            <div class="ep_stat_lbl">WPM</div>
          </div>
          <div class="ep_stat">
            <div class="ep_stat_val" style="color:#22c55e"><i class="fas fa-bullseye"></i> ${prec}%</div>
            <div class="ep_stat_lbl">Precisión</div>
          </div>
        </div>

        <button class="ep_btn_ver" data-u="${e.usuario}"><i class="fas fa-eye"></i> Ver Detalle</button>
      </div>`;
  }).join('');

  $('#ep_grid').html(html);
}

// ── MODALES ───────────────────────────────────────────────────────────────────
function _cerrarModales() { $('#ep_modales').html(''); }

function _modalDetalle(emp) {
  const nom  = emp.nombre || emp.usuario;
  const lecs = emp.completadas?.length || 0;
  const pct  = Math.round((lecs / 45) * 100);

  $('#ep_modales').html(`
    <div class="ep_modal_bg">
      <div class="ep_modal_card">
        <div class="ep_modal_hdr">
          <h3 class="ep_modal_title"><i class="fas fa-id-badge"></i> Ficha del Colaborador</h3>
          <button class="ep_modal_close"><i class="fas fa-times"></i></button>
        </div>
        <div class="ep_modal_body">
          <div style="display:flex;align-items:center;gap:2.5vh">
            <div class="ep_avatar" style="margin:0;width:8vh;height:8vh;font-size:3vh">${avatar(nom)}</div>
            <div>
              <h2 style="margin:0;font-size:var(--fz_l1);font-weight:900;color:var(--tx)">${nom}</h2>
              <div style="color:var(--tx3);font-size:var(--fz_m1);margin-top:.5vh">${emp.email || emp.usuario}</div>
            </div>
          </div>
          
          <div style="display:grid;grid-template-columns:repeat(3, 1fr);gap:1.5vh;margin-top:2vh">
            <div style="background:var(--bg4);border:1px solid var(--brd);border-radius:1.5vh;padding:2vh;text-align:center">
              <div style="font-size:var(--fz_l1);font-weight:900;color:var(--mco);line-height:1">${lecs}</div>
              <div style="font-size:var(--fz_s4);font-weight:700;color:var(--tx3);margin-top:.8vh;text-transform:uppercase">Lecciones</div>
            </div>
            <div style="background:var(--bg4);border:1px solid var(--brd);border-radius:1.5vh;padding:2vh;text-align:center">
              <div style="font-size:var(--fz_l1);font-weight:900;color:#f59e0b;line-height:1">${emp.wpmMax || 0}</div>
              <div style="font-size:var(--fz_s4);font-weight:700;color:var(--tx3);margin-top:.8vh;text-transform:uppercase">WPM Máx</div>
            </div>
            <div style="background:var(--bg4);border:1px solid var(--brd);border-radius:1.5vh;padding:2vh;text-align:center">
              <div style="font-size:var(--fz_l1);font-weight:900;color:#22c55e;line-height:1">${emp.precision || 0}%</div>
              <div style="font-size:var(--fz_s4);font-weight:700;color:var(--tx3);margin-top:.8vh;text-transform:uppercase">Precisión</div>
            </div>
          </div>

          <div class="ep_field" style="margin-top:2vh">
            <div style="display:flex;justify-content:space-between;font-size:var(--fz_s4);font-weight:700;color:var(--tx3);margin-bottom:.8vh">
              <span>Progreso del Curso</span><span>${pct}%</span>
            </div>
            <div style="width:100%;height:1.5vh;background:var(--brd);border-radius:10vh;overflow:hidden">
              <div style="height:100%;background:var(--mco);width:${pct}%;border-radius:10vh"></div>
            </div>
          </div>
        </div>
        <div class="ep_modal_foot">
          <button class="ep_btn_cancel">Cerrar</button>
        </div>
      </div>
    </div>`);
}

function _modalInvitar() {
  const optEq = _equipos.map(e => `<option value="${e.id}">${e.nombre || e.id}</option>`).join('');

  $('#ep_modales').html(`
    <div class="ep_modal_bg">
      <div class="ep_modal_card">
        <div class="ep_modal_hdr">
          <h3 class="ep_modal_title"><i class="fas fa-user-plus"></i> Registrar Colaborador</h3>
          <button class="ep_modal_close"><i class="fas fa-times"></i></button>
        </div>
        <div class="ep_modal_body">
          <div class="ep_field">
            <label>Correo corporativo</label>
            <input type="email" id="ep_inv_eml" class="ep_input" placeholder="ejemplo@empresa.com">
          </div>
          <div class="ep_field">
            <label>Nombre completo</label>
            <input type="text" id="ep_inv_nom" class="ep_input" placeholder="Nombre y Apellido">
          </div>
          <div class="ep_field">
            <label>Asignar a Departamento</label>
            <select id="ep_inv_eq" class="ep_input">
              <option value="">(General)</option>
              ${optEq}
            </select>
          </div>
        </div>
        <div class="ep_modal_foot">
          <button class="ep_btn_cancel">Cancelar</button>
          <button class="ep_btn_save" id="ep_save_invitar"><i class="fas fa-check"></i> Enviar Invitación</button>
        </div>
      </div>
    </div>`);

  $(document).off('click.ep2', '#ep_save_invitar').on('click.ep2', '#ep_save_invitar', function() {
    const eml = $('#ep_inv_eml').val().trim();
    const nom = $('#ep_inv_nom').val().trim();
    if (!eml || !nom) { Notificacion('El correo y nombre son obligatorios', 'warning'); return; }
    
    Notificacion(`Invitación enviada a ${eml} exitosamente`, 'success');
    _cerrarModales();
  });
}
