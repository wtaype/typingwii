...
// ── DATOS ──────────────────────────────────────────────────────────
const FEATURES = [
  { ico: 'fa-users-viewfinder', c: '#0EBEFF', t: 'Monitorea tu clase',       d: 'Visualiza en tiempo real el WPM, precisión y lecciones completadas de cada alumno. Todo en un panel centralizado.' },
  { ico: 'fa-paper-plane',      c: '#28a745', t: 'Envía mensajes directos',   d: 'Comunícate con cada estudiante desde la plataforma. Envía instrucciones, felicitaciones o correcciones personalizadas.' },
  { ico: 'fa-chart-bar',        c: '#fd7e14', t: 'Reportes de progreso',      d: 'Descarga reportes detallados de rendimiento por alumno o por grupo. Útil para evaluaciones y calificaciones.' },
  { ico: 'fa-award',            c: '#7000FF', t: 'Valida logros',             d: 'Cada certificado generado por tus alumnos queda registrado. Puedes verificar su autenticidad y validarlos.' },
  { ico: 'fa-list-check',       c: '#FF5C69', t: 'Asigna lecciones',          d: 'Define qué lecciones deben completar tus estudiantes y en qué orden. Personaliza el plan de estudio por grupo.' },
  { ico: 'fa-chalkboard-user',  c: '#FFD101', t: 'Perfil de instructor',      d: 'Cuenta con perfil de gestor con acceso privilegiado: ves datos de todos tus alumnos sin restricciones.' },
];

const VENTAJAS = [
  { ico: 'fa-clock', t: 'Ahorra tiempo de corrección', d: 'El sistema corrige automáticamente cada error. Tú ves el reporte, no tienes que evaluar manualmente.' },
  { ico: 'fa-fire',  t: 'Motiva con métricas reales',  d: 'Los alumnos ven su WPM subir lección a lección. La gamificación natural mantiene la motivación alta.' },
  { ico: 'fa-shield-halved', t: 'Datos seguros',       d: 'Los datos de tus alumnos están protegidos. Solo tú tienes acceso a los reportes de tu clase.' },
];

const PASOS_PROF = [
  { n: '01', ico: 'fa-id-badge',      t: 'Regístrate como gestor',    d: 'Solicita acceso de instructor. Nosotros activamos tu perfil con privilegios de gestor.' },
  { n: '02', ico: 'fa-user-plus',     t: 'Registra a tus alumnos',    d: 'Los estudiantes se registran solos o tú los invitas. Se vinculan a tu clase automáticamente.' },
  { n: '03', ico: 'fa-table-list',    t: 'Asigna el plan',            d: 'Elige las 12 lecciones del plan estándar o personaliza el orden según tu currículo.' },
  { n: '04', ico: 'fa-magnifying-glass-chart', t: 'Monitorea y evalúa', d: 'Sigue el progreso semanal de cada alumno con reportes descargables para tu portafolio docente.' },
];
...
<div class="pu_page pro_page">
  ...
  <section class="pu_hero pro_hero">
    ...
    <div class="pu_hero_inner">
      <div class="pu_hero_left">
        ...
        <p class="pu_sub">
          ${app} te da las herramientas para que tus alumnos aprendan mecanografía
        ...
        <div class="pro_quick_list">
          ${VENTAJAS.map(v => `
            <div class="pro_ql_item">
              <div class="pro_ql_ico"><i class="fas ${v.ico}"></i></div>
              <div><strong>${v.t}</strong><br><span>${v.d}</span></div>
            </div>`).join('')}
        </div>
      </div>
      <div class="pu_hero_right">
        ...
        <div class="pro_panel_card">
          ...
          <div class="pro_panel_body">
            ${[
              ['Ana García',   78, 97, 'Nivel 4'],
              ['Luis Pérez',   52, 91, 'Nivel 3'],
              ['María López',  41, 88, 'Nivel 2'],
              ['Carlos R.',    35, 84, 'Nivel 2'],
              ['Sofia T.',     29, 80, 'Nivel 1'],
            ].map(([n, wpm, prc, nv]) => `
              <div class="pro_student_row">
                <div class="pro_st_avatar"><i class="fas fa-user"></i></div>
                <div class="pro_st_info">
                  <div class="pro_st_name">${n}</div>
                  <div class="pro_st_nv">${nv}</div>
                </div>
                <div class="pro_st_wpm" style="color:#28a745">${wpm}</div>
                <div class="pro_st_prc" style="color:${prc >= 90 ? '#28a745' : '#fd7e14'}">${prc}%</div>
                <div class="pro_st_bar_w">
                  <div class="pro_st_bar" style="width:${(wpm/80*100).toFixed(0)}%;background:#28a745"></div>
                </div>
              </div>`).join('')}
  ...
  <section class="pu_sec pu_sec_alt">
    ...
    <div class="pu_pasos_grid">
      ${PASOS_PROF.map((p, i) => `
        <div class="pu_paso wi_fadeUp" style="--d:${i * .1}s;--paso_c:#28a745">
          <div class="pu_paso_num">${p.n}</div>
          <div class="pu_paso_ico" style="background:color-mix(in srgb,#28a745 14%,var(--bg4));color:#28a745"><i class="fas ${p.ico}"></i></div>
          <h3 class="pu_paso_t">${p.t}</h3>
          <p class="pu_paso_d">${p.d}</p>
          ${i < PASOS_PROF.length - 1 ? '<div class="pu_paso_arrow"><i class="fas fa-arrow-right"></i></div>' : ''}
        </div>`).join('')}
    </div>
  </section>
    ...
    <div class="pu_cta_card" style="--cta_c:#28a745;background:linear-gradient(135deg,#28a745,#1a7a32)">
      ...
      <div class="pu_cta_inner">
        <div class="pu_cta_txt">
          ...
          <h2>¿Quieres gestionar tu aula con ${app}?</h2>
...
export const init = () => {
  _obs = [
    ...
    wiVista('.pu_paso',      null, { anim: 'wi_fadeUp', stagger: 100 }),
    wiVista('.wi_fadeUp',    null, { anim: 'wi_fadeUp' }),
  ];
  console.log(`🏫 ${app} — Profesores listo`);
};
